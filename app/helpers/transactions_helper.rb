...
=begin
	def add_document(form_builder)
  	link_to_function "add", :id  => "add_document" do |page|
    	form_builder.fields_for :document, Document.new, :child_index => 'NEW_RECORD' do |document_form|
    	  ...
    	  page << "$('#{escape_javascript(html)}'.replace(/NEW_RECORD/g, new Date().getTime())).insertBefore($('#add_document')); $('#add_document').remove();"
   		 end
  	end
	end
    ...
    if form_builder.object.new_record?
      link_to_function("Remove this Document", "$(this).parent().remove()")
    else
