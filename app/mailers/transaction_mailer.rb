    ...
    unless transaction.blank?
      ...
      unless @recipient_emails.blank?
        ...
        if @transaction.other_domain_sender_email?
          mail(:to => @recipient_emails, :cc => @sender, :from => @sender, :subject => "#{@sender} has sent you an email: eDakia")
        else
          mail(:to => @recipient_emails, :from => @sender, :subject => "#{@sender} has sent you an email: eDakia")
        end
      else
        self.message.perform_deliveries = false
      end
